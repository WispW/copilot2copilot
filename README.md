...
2. **连接前预检**：中继模式下连接前会带令牌查询中继的在线名单（`GET /peers`），若自己的 id 已在线就直接不连，并提示改一个 id
3. **服务端权威拒绝**：中继对已在线 id 的新连接回 `4005` 拒绝（不再顶掉旧连接），客户端收到 `4001/4002/4004/4005` 这类不可自愈的关闭码时**停止自动重连**，等你在界面里改好后点「连接」重试
